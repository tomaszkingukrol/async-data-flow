...
async def foo(a, b): return a + b
async def bar(a, b): return a, b
async def baz(a, b): return {'a': a, 'b': b}
...
async def test_correct_input_mapping(func, input, input_map):
...
async def test_incorrect_input_mapping(func, input, input_map):
    f = amapper(func=func, input=input_map)
...
@pytest.mark.parametrize("func, input, output, output_map", 
                         ...
                         CORRECT_PARTIAL_OUTPUT_MAPPING + 
                         CORRECT_ZERO_OUTPUT_MAPPING)
async def test_correct_output_mapping(func, input, output, output_map):
...
async def test_incorrect_output_mapping(func, input, output, output_map):
    f = amapper(func=func, output=output_map)
    with pytest.raises(ArgsMapperError):
        await f(**input)
...
async def test_incorrect_args_mapping(func, input, input_map):
    f = amapper(func=func, input=input_map)
    with pytest.raises(ArgsMapperError):
        await f(**input)
